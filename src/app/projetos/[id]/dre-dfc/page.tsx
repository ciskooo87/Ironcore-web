...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const fechamento = (latest?.fechamento || {}) as Record<string, any>;

  const mainAction = !feeds.length
    ? "Gerar a primeira leitura contábil para abrir a camada financeira executiva."
    : Number(dre.resultadoOperacional || 0) < 0
      ? "Investigar o resultado operacional negativo e alinhar a leitura para fechamento e diretoria."
      : !fechamento.hasMonthlyClosure
        ? "Usar esta leitura para sustentar o próximo fechamento mensal."
        : "Manter a camada contábil atualizada para fechar o mês com narrativa consistente.";

  const mainRisk = Number(carteira.vencido || 0) > 0
    ? `Carteira vencida em ${br(carteira.vencido)} pressionando liquidez e leitura de risco.`
    : Number(dfc.saldoCaixaProxy || 0) < 0
      ? `Saldo de caixa proxy negativo em ${br(dfc.saldoCaixaProxy)}.`
      : Number(dre.resultadoOperacional || 0) < 0
        ? `Resultado operacional negativo em ${br(dre.resultadoOperacional)}.`
        : "Sem risco dominante explícito na leitura contábil atual.";

  return (
    <AppShell user={user} title="Projeto · DRE / DFC" subtitle="Camada financeira executiva para fechamento, diretoria e leitura gerencial do mês">
      ...
      <ProductHero
        ...
        description="A tela agora organiza geração da base contábil, comando financeiro, risco principal e trilha histórica num formato mais executivo."
      >
        <StatusPill label={`Feeds: ${feeds.length}`} tone={feeds.length > 0 ? "good" : "neutral"} />
        <StatusPill label={fechamento.hasMonthlyClosure ? "Com fechamento" : "Sem fechamento do mês"} tone={fechamento.hasMonthlyClosure ? "good" : "warn"} />
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr] mb-4">
        <section className="card">
          <div className="section-head"><h2 className="title">Comando financeiro</h2><span className="kpi-chip">prioridade do mês</span></div>
          <div className="mt-4 grid gap-3 md:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="rounded-[24px] border border-slate-800 bg-slate-950/30 p-4">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Receita bruta</div><div className={`mt-1 font-medium ${toneClasses(Number(dre.receitaBruta || 0))}`}>{br(dre.receitaBruta)}</div></div>
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Resultado operacional</div><div className={`mt-1 font-medium ${toneClasses(Number(dre.resultadoOperacional || 0))}`}>{br(dre.resultadoOperacional)}</div></div>
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Saldo caixa proxy</div><div className={`mt-1 font-medium ${toneClasses(Number(dfc.saldoCaixaProxy || 0))}`}>{br(dfc.saldoCaixaProxy)}</div></div>
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Carteira vencida</div><div className="mt-1 font-medium text-rose-300">{br(carteira.vencido)}</div></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="card">
          <div className="section-head"><h2 className="title">Leitura executiva</h2><span className="kpi-chip">camada do mês</span></div>
      ...
      <section className="card">
        ...
        <div className="mt-3 space-y-3 text-sm">
          {feeds.length ? feeds.map((item) => (
            <div key={item.id} className="rounded-[24px] border border-slate-800 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <StatusPill label={((item.payload as any)?.fechamento?.hasMonthlyClosure) ? "com fechamento" : "sem fechamento"} tone={((item.payload as any)?.fechamento?.hasMonthlyClosure) ? "good" : "warn"} />
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2 text-sm md:grid-cols-4">
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Receita</div><div className="mt-1 font-medium text-white">{br((item.payload as any)?.dre?.receitaBruta)}</div></div>
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Resultado operacional</div><div className={`mt-1 font-medium ${toneClasses(Number((item.payload as any)?.dre?.resultadoOperacional || 0))}`}>{br((item.payload as any)?.dre?.resultadoOperacional)}</div></div>
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Caixa proxy</div><div className={`mt-1 font-medium ${toneClasses(Number((item.payload as any)?.dfc?.saldoCaixaProxy || 0))}`}>{br((item.payload as any)?.dfc?.saldoCaixaProxy)}</div></div>
                <div className="rounded-2xl border border-slate-800 p-3"><div className="text-xs text-slate-400">Carteira vencida</div><div className="mt-1 font-medium text-rose-300">{br((item.payload as any)?.carteira?.vencido)}</div></div>
              </div>
            </div>
          )) : <EmptyState title="Sem alimentação contábil gerada ainda" description="Assim que a base contábil for gerada, a trilha financeira vai aparecer aqui para sustentar fechamento e diretoria." />}
        </div>
      </section>
    </AppShell>
  );
