...
export async function POST(req: Request, ctx: { params: Promise<{ code: string }> }) {
  ...
  const runQ = await dbQuery<{ business_date: string; status: string; summary: Record<string, unknown> }>(
    `select business_date::text, status, summary from routine_runs where id=$1 and project_id=$2 limit 1`,
  ...
  const routineStatus = String(run?.status || '-');
  const gatingStatus = String(op.gatingStatus || '-');
  const isBlocked = routineStatus === 'blocked' || gatingStatus === 'bloqueado';

  if (decision === 'aprovado' && isBlocked) {
    return NextResponse.redirect(publicUrl(req, `/projetos/${code}/movimento-diario/?error=blocked_state_requires_action`));
  }
  ...
  const validationStatus = decision === "aprovado"
    ? "concluido"
    : decision === "bloquear"
      ? "bloqueado"
      : "em_execucao";

  const movementStatus = decision === "aprovado"
    ? "concluido"
    : decision === "bloquear"
      ? "bloqueado"
      : isBlocked
